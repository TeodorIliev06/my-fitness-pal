package com.fmi.myfitnesspal.command.food;

import com.fmi.myfitnesspal.command.ExecutableCommand;
import com.fmi.myfitnesspal.constants.GlobalConstants;
import com.fmi.myfitnesspal.exception.InvalidCommandException;
import com.fmi.myfitnesspal.food.EatingTime;
import com.fmi.myfitnesspal.food.Food;
import com.fmi.myfitnesspal.food.FoodDiary;
import com.fmi.myfitnesspal.food.Recipe;
import com.fmi.myfitnesspal.food.RecipeId;
import com.fmi.myfitnesspal.food.RecipePool;

import java.time.LocalDate;
import java.util.List;

import static com.fmi.myfitnesspal.command.utility.CommandUtilities.parseEatingTime;
import static com.fmi.myfitnesspal.command.utility.CommandUtilities.validateArgumentsCount;
import static com.fmi.myfitnesspal.utility.DateHelper.parseDate;
import static com.fmi.myfitnesspal.utility.NumberParser.parseDouble;

public final class AddRecipeCommand implements ExecutableCommand {

    private static final String COMMAND_NAME = "add-recipe";
    private static final int ARGUMENTS_COUNT = 5;
    private final FoodDiary foodDiary;
    private final RecipePool recipePool;

    public AddRecipeCommand(FoodDiary foodDiary, RecipePool recipePool) {
        this.foodDiary = foodDiary;
        this.recipePool = recipePool;
    }

    @Override
    public String execute(List<String> arguments) throws InvalidCommandException {
        validateArgumentsCount(arguments, ARGUMENTS_COUNT);

        LocalDate consumptionDate = parseDate(arguments.get(0));
        EatingTime eatingTime = parseEatingTime(arguments.get(1));
        RecipeId recipeId = new RecipeId(arguments.get(2), arguments.get(3));
        double numberOfServings = parseDouble(arguments.get(4));

        Recipe targetRecipe = this.recipePool.getRecipe(recipeId);
        Food foodPerServing = targetRecipe.toFoodPerServing();
        this.foodDiary.addFood(consumptionDate, eatingTime, foodPerServing, numberOfServings);

        return GlobalConstants.SUCCESSFULLY_ADDED_RECIPE_MESSAGE;
    }

    @Override
    public String name() {
        return COMMAND_NAME;
    }

    @Override
    public String getHelp() {
        return "Usage: " + COMMAND_NAME + " <date> <eatingTime> <recipeName> <recipeDescription> <numberOfServings>";
    }
}
